...
Knot CalculateMoveVecForPointAngle(Grid* grid,int idMoveKnot,std::vector<Edge>& edges)
{
...
void CalculateMoveVecForPointsAngle(Grid* grid,const std::vector<int>& queueIdKnots, MbSplineSurface* srfNurbs)
{
	...
	for (auto idKnot : queueIdKnots)
	{
		...
		for (auto incEdge : grid->aPoints[idKnot].incEdgesId)
		{
			...
			//	}
			//}
			newPlaceKnot.push_back(CalculateMoveVecForPointAngle(grid, idKnot, edges));
...
double DiffTargetAnglesAndIdeal(const Knot& knot)
{
	...
	if (knot.targetAngle.size != 4 || knot.targetAngle.size != 3 || knot.targetAngle.size != 2
		|| knot.targetAngle.size != 5)
	{
		throw - 1;
	}
...
double CalcFxy(Grid* grid, int* indexDf = nullptr)
{
	double res = 0;
	for (auto& knot : grid->aPoints)
	{
